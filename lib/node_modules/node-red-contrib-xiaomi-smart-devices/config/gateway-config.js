  ...
  function XiaomiGatewayConfigNode(config) {
    ...
    this.sendCommand = (input) => {
      if (this.key && this.currentToken) {
        ...
        const self = this;
        socket.send(message, 0, message.length, self.port, self.address, function () {
          console.info(`Sending message '${message}' to '${self.address}'`);
        });
      } else {
    ...
    socket.on('message', (message, rinfo) => {
      ...
      if (process.env.DEBUG === 'on') console.log(`Multicast message - ${JSON.stringify(msg)}`);
